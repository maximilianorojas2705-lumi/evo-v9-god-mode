# Alice Agent - Development Guide

Instructions for AI coding assistants and developers working on the alice-agent codebase.

**Never give up on the right solution.**

## What Alice Is

Alice is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Alice ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`alice native`, `alice setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `ALICE_HOME`. Mocks hide integration bugs.
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `alice native`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.alice/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `alice <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `alice webhook`, `alice cron`, `alice native`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.alice/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.alice/alice-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── alice_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── alice_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   └── builtin_hooks/    # Extension point for always-registered gateway hooks (none ship