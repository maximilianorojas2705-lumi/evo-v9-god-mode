...
MAPA = {"BTCUSDT":"bitcoin","ETHUSDT":"ethereum","SOLUSDT":"solana","BNBUSDT":"binancecoin","XRPUSDT":"ripple","BTC":"bitcoin","ETH":"ethereum","SOL":"solana","BNB":"binancecoin","XRP":"ripple","DOGE":"dogecoin"}
...
def webhook():
    ...
    if "callback_query" in j:
        ...
        if act.startswith("senal_"):
            sym=act.replace("senal_",""); precio,cambio=get_precio(sym)
            if precio:
                ...
                send(chat_id, f"📈 *{sym}*\nPrecio: ${precio:.2f}\n({cambio:.2f}%)\n*{senal}*\nSens: {mem['sensibilidad']:.2f} | Total:{mem['senales_dadas']} | Supabase ✅", btns)
            else: send(chat_id, "❌ Error precio, reintenta en 10s")
            ...
            send(chat_id, "📋 Elegi moneda (CoinGecko gratis):", btns)
...
def auto():
    ...
    if precio:
        ...
        if oport: send(chat_id, f"🚨 *AUTO-ALERTA EVO*\nBTC ${precio:.2f} ({cambio:.2f}%)\n{senal}\nTe aviso solo porque es oportunidad."); return f"Sent {senal}",200
    return "No oport",200
