...
def ask_groq(prompt):
    if not GROQ_API_KEY:
        return "def tool(): return 'Falta GROQ'"
...
@app.route("/")
def home():
    return "EVO V9 - CLONADOR DE IA LIVE", 200
    ...
    for name, repo_path in EVOLUTION_REPOS.items():
        try:
            ...
            if r.status_code!= 200:
                logs.append(f"X {name}: {r.status_code}")
                continue
            count = 0
            for f in r.json():
                if count >= 3:
...
'''
    ...
    return "<h1>GOD ENGINE CREADO</h1><br>GOD_ENGINE_clone_and_improve.py creado - clona IAs y las mejora estilo Godel Agent"
...
def telegram_webhook():
    try:
        ...
        if not chat_id or not text:
            return "ok", 200
        if text.startswith("OK") and "CEREBROS" in text:
            return "ok", 200
        low = text.lower()
